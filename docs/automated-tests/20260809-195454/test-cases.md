...
- Compat: CreateNumericRuleFormatter hands back an object with FormatNumber
...
- State: SetPreview flips the flag and publishes PREVIEW_CHANGED once
- State: SetPreview no-ops when the flag is already in the requested state
- State: SetPreview coerces truthy values to a boolean before comparing
- State: SetPreview is the only sender of PREVIEW_CHANGED
...
- Defaults: the shipped text font is the LSM key the addon registers
- Defaults: the shipped sort column is a stat the window ships enabled
- Defaults: the shipped session type is Current, the live pull
...
- Defaults: global carries the schema version and nothing else
...
- ShouldShow: preview overrides context, so a window can be positioned anywhere
...
- Format.Number goes through the native NumericRuleFormatter
- Format.Invalidate drops the cached formatter instance
- Format.Number('full') does NOT abbreviate and never calls the formatter
...
- Format.Rate appends the unit to a secret with `..`, never table.concat
...
- BuildPreview produces Build's shape, so the renderer has no preview branch
- BuildPreview is deterministic — a jittering preview cannot be laid out against
- BuildPreview reaches no meter API at all
...
- ShouldShow's ladder reads master enable, then preview, then context
...
- The header line reads 'Preview' while preview data is on screen
- Preview data never reaches the provider
...
- A rate-capable column renders BOTH the total and the per-second figure
...
- The name cell scales its bar to the SORT column
...
- alternatingBackground stripes the even rows
...
- The avoidable column tags Avoidable, Deadly and Overkill
...
- SetLocked flips every window and takes preview with it
- IsLocked is false the moment any one window is unlocked
- SetPreview routes through core/State.lua and marks every window dirty
...
- Slash: `preview` sets and toggles through the registry
