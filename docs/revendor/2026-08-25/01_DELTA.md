# 01 — Delta: MultiMeters vs LibKa0s v1.15.0
...
```sh
grep -in 'bundles' MultiMeters/CLAUDE.md
```
...
```sh
grep -hoE 'local +(MAJOR, )?[A-Z_]*MINOR *= *("[^"]+", *)?[0-9]+' MultiMeters/libs/LibKa0s/*.lua
```
...
```sh
diff -r --strip-trailing-cr <scratch>/LibKa0s MultiMeters/libs/LibKa0s   # 0 lines
diff -r                     <scratch>/LibKa0s MultiMeters/libs/LibKa0s   # 0 lines
diff -r --strip-trailing-cr <scratch>/testkit MultiMeters/tests/_kit     # 0 lines
diff -r                     <scratch>/testkit MultiMeters/tests/_kit     # 0 lines
```
...
```sh
grep -rnoE 'LibStub\("LibKa0s-[A-Za-z]+-1\.0", true\)' MultiMeters --include='*.lua' | grep -v '/libs/' | grep -v '/tests/'
```

- `MultiMeters/core/CoreSetup.lua:88` — Core
- `MultiMeters/core/DebugLogSetup.lua:227` — DebugLog
- `MultiMeters/core/EnvSetup.lua:71` — Env
- `MultiMeters/core/MediaSetup.lua:72` — Media
- `MultiMeters/core/PerfSetup.lua:41` — Perf
- `MultiMeters/modules/Export.lua:81` — Widgets
- `MultiMeters/settings/OptionsSetup.lua:34` — Options
- `MultiMeters/settings/Slash.lua:110` — Slash

Majors present in the payload with **no lookup anywhere in this addon**: `Item`, `Pool`.

## 3f. Kit revision and the pairing rule

```sh
grep -n 'Kit.VERSION' <scratch>/testkit/framework.lua MultiMeters/tests/_kit/framework.lua
```
...
| Gate | Command | Result |
|---|---|---|
| Lint | `luacheck .` | 0 warnings / 0 errors in 47 files |
| Tests | `lua tests/run.lua` | 1257 passed, 0 failed, 0 skipped |
