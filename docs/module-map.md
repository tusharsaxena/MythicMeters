...
└── settings/
    ├── Schema.lua      — NS.Schema (137 rows) and the write seam: GetSetting,
...
| `Frame.lua` | The Frame page (24 rows across six tabs). Pure schema — the header controls moved to Header and "Reset position" to General | a page registration | `NS.Helpers` |
| `Header.lua` | The Header page (24 rows across five tabs): the title bar's own text, and the meter's controls split by what they act on (Window buttons, Meter buttons) and how they are drawn (Button style). The column-header strip moved to the Columns page it labels. Rows are stored at `window.frame.*` / `window.header.*`. Pure schema | a page registration | `NS.Helpers` |
| `Bars.lua` | The Bars page (27 rows across six tabs): the bar, its background, its border, the cell's two text slots and the row icon. The Text and Icons pages folded in here; their PATHS did not move with them. Pure schema | a page registration | `NS.Helpers` |
| `Tooltip.lua` | The Tooltip page (27 rows across six tabs). Pure schema | a page registration | `NS.Helpers` |
