...
| `General.lua` | The General page (17 rows across two tabs) and both of the addon's bespoke buttons: the master enable, the minimap toggle, the two addon-wide data settings, the two session-only checkboxes (test mode, debug console), the three hidden export preferences and the eight generated statistic-colour swatches — plus the reset-everything confirmation and "Reset position", the one per-window control on the page. Also hosts the **reset-meter-data dialog**, which has no button on any page: the header's own reset control is the one way to open it, and it routes to `NS.Provider.Reset` rather than to the Compat shim | a page registration, `NS.ShowResetMeterData` | `NS.Helpers`, `NS.State`, `NS.DebugLog`, `NS.WindowManager.ResetPosition`, `NS.Provider.Reset` |
