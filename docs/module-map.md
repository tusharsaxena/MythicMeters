...
│                         sort modes and the sort freeze, the row cap, and the
...
| `Aggregator.lua` | AceAddon | The join, the filter, pet folding, ordering, the freeze, the row cap, `percent` | `NS.Aggregator` — `Build`, `BuildPreview`, `ApplyRowLimit` | `NS.Provider`, `NS.Roster`, `NS.Secrets`, `NS.State.Cache("Aggregator")`. Subscribes `RESTRICTION_CHANGED`, `METER_RESET`, `PROFILE_CHANGED` |
