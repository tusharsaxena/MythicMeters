...
| `ColumnBlocks.lua` | A reorderable list of blocks — handle, state glyph, label, and a rule where the enabled ones stop. Knows nothing about statistics; takes `items` and answers with indices. Loads **before** `Columns.lua`. A generic options widget living outside LibKa0s is a [documented deviation](ARCHITECTURE.md#documented-deviations) (issue #21) | `NS.ReorderableBlocks(ctx, spec)`, `NS.BLOCK_HEIGHT`, `NS.BLOCK_STRIDE` | `NS.Helpers.EnsureScroll`, `NS.AceGUI`, `NS.Icon` |
| `Columns.lua` | The column editor — one block per statistic, ticked or not, dragged into order. **No schema rows**: every write hands the seam a freshly built whole array, and every mutation re-checks combat | a page registration | `NS.ReorderableBlocks`, `NS.SetByPath("window.columns", …)`, `NS.Constants.STATS` |
